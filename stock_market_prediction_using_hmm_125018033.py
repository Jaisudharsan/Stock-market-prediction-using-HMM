# -*- coding: utf-8 -*-
"""Stock market prediction using HMM - ML project

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1z_ufoFGq_yjZ-VlaE50i2m3_B-7s7Clx
"""
...
def augment_features(dataframe):
    # Access columns using MultiIndex
    fracocp = (dataframe[('Close', '^GSPC')] - dataframe[('Open', '^GSPC')]) / dataframe[('Open', '^GSPC')]
    frachp = (dataframe[('High', '^GSPC')] - dataframe[('Open', '^GSPC')]) / dataframe[('Open', '^GSPC')]
    fraclp = (dataframe[('Open', '^GSPC')] - dataframe[('Low', '^GSPC')]) / dataframe[('Open', '^GSPC')]

    # Create new DataFrame with calculated columns and set index
...
actual_close = test_data.iloc[0:num_days_to_predict]['Close'].values
predicted_close_prices = np.array(predicted_close_prices)

if len(actual_close) != len(predicted_close_prices):
    raise ValueError("The length of actual and predicted close prices must match.")

# Calculate Absolute Error
ae = abs(actual_close - predicted_close_prices)
...
for num_component in n_components_values:
    ...
    # Convert predicted_close_prices to a numpy array
    predicted_close_prices = np.array(predicted_close_prices)

    # Compute Mean Absolute Error (MAE) and append to mae_num_components
    mae = abs(test_data.iloc[0:num_days_to_predict]['Close'].values - predicted_close_prices).mean()
    mae_num_components.append(mae)
...
import itertools
from tqdm import tqdm
from hmmlearn.hmm import GaussianHMM  # Assuming GaussianHMM is imported
...
for num_step in num_steps_values:
    ...
    # Convert predicted_close_prices to a NumPy array for element-wise subtraction
    predicted_close_prices = np.array(predicted_close_prices)

    # Compute Mean Absolute Error (MAE) and append to mae_num_steps
    mae = abs(test_data.iloc[0:num_days_to_predict]['Close'].values - predicted_close_prices).mean()
